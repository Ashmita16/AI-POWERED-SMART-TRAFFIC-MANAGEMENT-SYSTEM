...
EMERGENCY_CLASSES = [3, 4, 5]  # Motorcycle, Bus, Truck (Ambulance/Fire/Police)
...
def detect_forward_moving_cars(frame, tracked_objects):
    ...
    car_detections = [detection[:5].tolist() for detection in detections if int(detection[5]) == 2]  # Class 2 is 'car'
    ...
    for obj in tracked_objects_arr:
        ...
        if track_id in tracked_objects and cy > tracked_objects[track_id][1]:  # Moving forward
            forward_count += 1
...
if option == "Upload Video":
    st.subheader("Video Upload - Detection of Moving Cars & Adjustment of Signals")
    ...
    st.subheader("Image Upload - Detection of Emergency Vehicles")
